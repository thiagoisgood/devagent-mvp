...
function cleanJson(str) { return str.trim().replace(/^```json\n/, '').replace(/\n```$/, ''); }
export async function askAI(sys, usr, model) {
    if (model === 'qwen') {
        const res = await fetch('https://dashscope.aliyuncs.com/compatible-mode/v1/chat/completions', {
            ...
            body: JSON.stringify({ model: "qwen-plus", messages: [{role:"system",content:sys},{role:"user",content:usr}], response_format: {type:"json_object"} })
        });
