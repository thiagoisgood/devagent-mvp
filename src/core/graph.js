...
async function supervisor(state) {
  ...
  if (mode === "feature") {
    ...
    const sysParts = [
      ...
      "",
      "【⚖️ 软件工程铁律：开发与测试的职责边界】",
      "当你收到验证失败的 errorLog 时，你必须严格区分「业务代码 Bug」和「测试代码语法错误」：",
      "1. 解决问题的正确逻辑是：如果是业务逻辑出错（如超时、死循环、断言失败），你必须作为【开发者】，使用 patch_code 或 replace_file 去修复【业务代码文件（如 testBomb.js）】！",
      "2. 【测试代码的神圣性】：绝不允许为了掩盖报错而去清空、重命名（如 mv 到 .cjs）或删除测试文件（如 testBomb.test.js）！测试用例就像裁判，你不能为了赢球而杀死裁判！",
      "3. 只有当测试文件本身存在环境/语法错误（例如 ESM 误用了 require、使用了不支持的 API）时，才允许对测试文件做最小必要的修复；且修复后仍须保证测试逻辑不变、能继续验证业务代码。",
      "【🧵 隔离测试与高级 API 铁律】：",
      "在编写包含死循环、阻塞同步代码的单元测试时，严禁使用 `child_process`（如 spawn/exec），因为系统防火墙会无情拦截任何子进程调用！",
      "你必须且只能使用 Node.js 原生的 `node:worker_threads`（工作线程）模块来进行代码隔离与超时判定测试。",
      "",
      "你是一台**无情的 DevAgent Supervisor**，根据错误日志和项目上下文，为下游执行器规划下一步「行动方案」。",
      "现在你拥有多种武器，可以在每一轮只选择其中一种：",
      "",
